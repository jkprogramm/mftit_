(function($){
    $.fn.extend({
        jQuizler: function(questions) {
            ...
            if (questions == null)
                throw 'Эч кандай суроолор берилген жок.';
            ...
            return this.each(function(){
                ...
                $.each(questions, function(index, question){
                    ...
                    html += "<div class=\"question-number\"> " + (index + 1) + ' - суроо ' +  "</div><div style=\"margin-bottom:10px; clear:both\"></div>";
                    ...
                    html += "<ul class=\"answers list-group\">";

                    for (var i = 0; i < question.answers.length; i++)
                        html += "<li class=\"list-group-item\">" + question.answers[i] + "</li>";

                    html += "</ul>";
                    html += "<div class=\"nav-container\">";

                    html += "<div class=\"notice alert alert-danger\">Жоопту белгилеңиз</div>";

                    if (index != 0) {
                        html += "<button class=\"prev btn btn-info btn-small\"><i class='fa fa-arrow-left'></i> Мурунку</button>";
                    }

                    if (index != questions.length-1) {
                        html += "<button class=\"next btn btn-success btn-small\">Кийинки <i class='fa fa-arrow-right'></i></button>";
                        html += "<div style=\"clear:both\"></div>";

                    } else {
                        html += "<button class=\"final btn btn-success btn-small\">Аяктоо <i class='fa fa-check'></i></button>";
                        html += "<div style=\"clear:both\"></div>";
                    }
                ...
                html += "<div class=\"progress progress-info progress-striped active\">";
                ...
                $(".final").click(function(e){
                    ...
                    if (userAnswer.index() == -1 && !reviewQuiz) {
                        ...
                        var resultHTML = "<h3 style=\"text-align: center\">Жыйынтык</h3>";
                        ...
                        $.each(questions, function(index, question){
                            ...
                            if (element.index() == question.correct) {
                                ...
                                buttonsHTML += "<button class=\"btn btn-success\"> " + (index+1) + " - суроо <i class='fa fa-check'></i></button>";
                                rightAnswers++;
                            } else {
                                element.removeClass("btn-info");
                                element.addClass("btn-danger");

                                buttonsHTML += "<button class=\"btn btn-danger\"> " + (index+1) + " - суроо <i class='fa fa-times'></i></button>";
                        ...
                        resultHTML += "<p style=\"margin: 14px 0px\">Сиз " + Math.round(((rightAnswers * 100) / questions.length) * 100) / 100 + "% суроого туура жооп бердиңиз.</p>";
                        resultHTML += buttonsHTML;
                        resultHTML += "<p style=\"margin-top:25px; text-align: center\"><button class=\"btn btn-small tostart\">Суроолорду көрүү</button></p>";
